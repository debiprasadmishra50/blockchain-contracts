...
contract TestUniV2FlashSwap is IUniswapV2Callee {
    ...
    function testFlashSwap(address _tokenBorrow, uint256 _amount) external {
        ...
        require(pair != address(0), "no pair present");
        ...
        // make the swap
        IUniswapV2Pair(pair).swap(amount0Out, amount1Out, address(this), data); // if the last data is empty uniswap will do a regular swap, if it's not empty, uniswap will do a Flash-Swap
    }
    ...
    function uniswapV2Call(
        ...
        require(msg.sender == pair, "no pair present");
        require(_sender == address(this), "no sender is present");
